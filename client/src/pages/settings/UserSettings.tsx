...
const [pushEnabled, setPushEnabled] = useState(false);
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        setAuth({ ...user, ...res.data }, useAuthStore.getState().accessToken!, useAuthStore.getState().refreshToken!);
      }
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      await settingsService.changePassword(passwords.current_password, passwords.new_password);
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="bg-red-500/20 text-red-400 p-4 rounded-xl text-sm">{error}</div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-4">
              ...
              <button type="submit" className="bg-[#F5C518] text-[#0D0F3C] font-semibold px-6 py-3 rounded-xl hover:bg-yellow-400 transition-all">
                Save Changes
              </button>
            </form>
          </div>
      ...
      {activeTab === 'preferences' && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <form onSubmit={handlePreferencesUpdate} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-white/70 text-sm mb-1">Pomodoro Focus (mins)</label>
                <input
                  type="number"
                  min="5"
                  max="60"
                  value={preferences.pomodoro_focus_mins}
                  onChange={(e) => setPreferences({ ...preferences, pomodoro_focus_mins: parseInt(e.target.value) })}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5C518]"
                />
              </div>
              <div>
                <label className="block text-white/70 text-sm mb-1">Pomodoro Break (mins)</label>
                <input
                  type="number"
                  min="1"
                  max="30"
                  value={preferences.pomodoro_break_mins}
                  onChange={(e) => setPreferences({ ...preferences, pomodoro_break_mins: parseInt(e.target.value) })}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#F5C518]"
                />
              </div>
            </div>
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-white/70 text-sm">Academic Analytics Notifications</span>
                <input
                  type="checkbox"
                  checked={preferences.notif_academic_analytics}
                  onChange={(e) => setPreferences({ ...preferences, notif_academic_analytics: e.target.checked })}
              ...
              <label className="flex items-center justify-between cursor-pointer">
                ...
                <input
                  ...
                  onChange={(e) => setPreferences({ ...preferences, notif_study_reminders: e.target.checked })}
                  className="accent-[#F5C518] w-5 h-5"
                />
              </label>
            </div>
            ...
            <div className="border-t border-white/10 pt-4">
              <h3 className="text-white/70 text-sm font-medium mb-3">Browser Push Notifications</h3>
              <button
                type="button"
                onClick={handleEnablePush}
                disabled={pushLoading}
                className="bg-[#F5C518] text-[#0D0F3C] font-semibold px-4 py-2 rounded-xl text-sm hover:bg-yellow-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
